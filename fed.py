...
@click.option("--fedprox-pmu", type=click.FloatRange(0.0, 1.0, min_open=True), default=None)
def client(
    server_address: str,
    wandb_project: str,
    wandb_group: str,
    ...
    fedprox_pmu: float | None,
) -> None:
    time.sleep(5)

    if fedprox_pmu is None:
        fedprox_pmu = 0.0
    ...
    wandb.init(
        ...
        name=wandb_name,
        config=dict(
            **get_wandb_config_data((train_paths,), test_paths, None, None),
            fedprox_pmu=fedprox_pmu,
            online_cuts=online_cuts,
            online_additive=online_additive,
        ),
    )
